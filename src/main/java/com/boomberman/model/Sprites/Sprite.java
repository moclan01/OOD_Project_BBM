...
public class Sprite {
    ...
    private static final int TRANSPARENT = 0xffff00ff; // địa chỉ để lưu giá trị
    ...
    //trả về kích thước được phóng đại
    ...
    /*
     tạo ra một đối tượng Image mới có kích thước lớn hơn từ một đối tượng WritableImage được cung cấp.
     Đối tượng Image mới này có thể được sử dụng để hiển thị hình ảnh phóng to trên màn hình
     hoặc để sử dụng trong các tính năng khác trong trò chơi.
     */
    private Image upscale(WritableImage inputImage){
        ...
        //tạo ra một đối tượng WritableImage mới có kích thước lớn hơn
        WritableImage outputImage = new WritableImage(width * SCALED_FACTOR, height * SCALED_FACTOR);
        //đọc các giá trị màu của các pixel trong hình ảnh gốc.
            ...
            for (int j = 0; j < height; j++){
                ...
                for (int scaledY = 0; scaledY < SCALED_FACTOR; ++scaledY) {
                    for (int scaledX = 0; scaledX < SCALED_FACTOR; ++scaledX) {
                        pixelWriter.setArgb(i * SCALED_FACTOR + scaledX, j * SCALED_FACTOR + scaledY, rgb);
                    }
                }
            }
        }
